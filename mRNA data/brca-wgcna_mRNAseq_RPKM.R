setwd("E:/survival/000000DrIsabelle_clinical/mRNA")
rm(list = ls())
...
library(ggplot2)
library(data.table)
#fpkm1<-read.table("BRCAMerged_after_WAVE_color.csv", sep="\t", 
#header = T, fill = T, stringsAsFactors=FALSE)
fpkm2= read.csv("BRCA_mRNAseq_RPKM.csv",header = T, fill = T, stringsAsFactors=FALSE)
#fpkm2= read.csv("BRCAMerged_after_WAVE_color.csv",header =FALSE) #这三句等于上面一句
#colnames(fpkm2) = fpkm2[1, ] # the first row will be the header 第一行提升作为头，
#fpkm2 = fpkm2[-1, ] #去掉第一行
s1 = nrow(fpkm2)
s2 = ncol(fpkm2)
# 切换工作目录如果有必要的话
...
if(F){
  ...
  a=read.table("BRCAMerged_after_WAVE_color.tsv", sep="\t", 
               header = T, fill = T, stringsAsFactors=FALSE)
  library(reshape2)
...
if(T){
  ...
  #datExpr2=filter(datExpr1,datExpr1!=0)
  datExpr3=datExpr1[rowSums(datExpr1==0)==0,]
  datExpr4<- transpose( datExpr3, fill=NA, ignore.empty=FALSE)
  datExpr <- datExpr4 
  
  
  datTraits5=fpkm2[,5]
  datTraits5[is.na(datTraits5)]<-5
  library(stringr)
  datTraits6=as.character(datTraits5,collapse='')
...
if(T){
  ...
  #针对前面构造的样品矩阵添加对应颜色
  sample_colors <- numbers2colors(as.numeric(factor(datTraits5)), 
...
if(T){
  nGenes = ncol(datExpr)
  nSamples = nrow(datExpr)
  design=model.matrix(~0+ datTraits6)
  #colnames(design)=levels(datTraits6)
  colnames(design)=c("datTraits61", "datTraits62", "datTraits63", "datTraits64", "datTraits65")
...
write.csv(MMPvalue,file = 'MMPvalue_mRNAseq_RPKM.csv')
...
if(T){
  ...
  dissTOM = 1-TOMsimilarityFromExpr(datExpr, power = 29); 
  plotTOM = dissTOM^7; 
  diag(plotTOM) = NA; 
...
MET1 <- cbind(fpkm2[,1],MET)
write.csv(MET1,file = 'MET_moduleEigengenes_mRNAseq_RPKM_0.csv')
MEs1 <- cbind(fpkm2[,1],MEs)
write.csv(MEs1,file = 'MEs_moduleEigengenes_mRNAseq_RPKM_0.csv')
## step 8 
if(T){
  # Select module
  module = "brown";
